...
This app lives in the standalone `den-desktop` repository. It bundles and runs its own local UI instead of serving or iframing Den web from the Den server.
...
```bash
# From any checkout of den-desktop, or after copying the script to ~/bin:
...
1. Ensures a local checkout exists at `${DEN_DESKTOP_REPO_DIR:-~/dev/den-desktop}`.
...
```text
~/dev/den-desktop                                  local source checkout
...
```bash
DEN_DESKTOP_REPO_DIR="$HOME/apps/den-desktop" \
